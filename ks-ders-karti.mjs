...
t("D16/D25 WA mesajında slot numarası YOK + gerçek aralık VAR", (() => { const kayitliD = DB.dersler; const kayitliF = ui.filtre; const kayitliA = ui.anchor; DB.dersler = [birebir]; ui.filtre = "tumu"; const m = ogrenciMesajMetni(ogr.id); DB.dersler = kayitliD; ui.filtre = kayitliF; ui.anchor = kayitliA; return !!m && !m.includes("8 · 15:30-16:10") && m.includes("15:30 - 16:10"); })());
...
/* DÖNGÜ-17 sözleşme değişikliği: öğretmen GÜNLÜK PNG kartında footer KALDIRILDI; saat başlıkları artık 3 satırlı (no/b/e — "8 · " tek-satır biçimi yok) ve Mola kolonu EKLENDİ. Öğretmen TEK-DERS kartındaki footer/marka/saat KORUNDU. Eski D16 assertion bu spec ile güncellendi. */
t("D17 öğretmen günlük PNG: footer YOK + Mola VAR + 12 kolon; TEK-DERS kartı footer + '8 · ' KORUNDU", (() => { const gun = (() => { const d = new Date(); d.setDate(d.getDate() - ((d.getDay() + 6) % 7) + 7); return d.getFullYear() + "-" + String(d.getMonth() + 1).padStart(2, "0") + "-" + String(d.getDate()).padStart(2, "0"); })(); const kayitli = DB.dersler; DB.dersler = [birebir]; const h = dersKartiOgrtGunlukHTML(ogrt.id, gun); const tek = dersKartiOgrtHTML(birebir); DB.dersler = kayitli; return !h.includes("Bu kart YKS Birebir Takip tarafından oluşturuldu") && h.includes(">Mola<") && !tek.includes(">Mola<") && tek.includes("Bu kart YKS Birebir Takip tarafından oluşturuldu") && tek.includes("8 · "); })());
