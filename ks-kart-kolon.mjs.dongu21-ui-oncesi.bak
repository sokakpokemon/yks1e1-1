...
(async () => {
  ...
  /* A) Boot sonrası wrapper varlığı + teklik */
  {
    ...
    /* GERÇEK-DAL FIXTURE (site #45): onarım İKİNCİ kez çökertilir → catch dalı GERÇEKTEN ateşlenir;
       ...
       iki dal da bu koşumda koşulur. */
    try {
      const originalOnarim = api.kartKolonOnar;
      api.kartKolonOnar = () => { throw new Error("GERÇEK-DAL-ZORLAMA-2"); };
      try { api.kartKolonOnar(); t("kart-kolon fixture: İKİNCİ onarım çökertilmedi (beklenmedik)", false); }
      catch (e2) { t("kart-kolon fixture: onarım catch dalı GERÇEKTEN ateşlendi (2. onarım çökertilip yakalandı)", String(e2 && e2.message).includes("GERÇEK-DAL-ZORLAMA-2")); }
      finally { api.kartKolonOnar = originalOnarim; }
    } catch (dis) { t("kart-kolon fixture: dış çökme olmadı", false, String(dis && dis.message)); }
  }
