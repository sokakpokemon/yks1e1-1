...
function ogrenciMesajMetni(ogrenciId) {
  ...
  /* WA-SABLON-YAMASI: metin parçaları DB.ayarlar.whatsappSablon'dan; boşsa bugünkü varsayılan byte-birebir. */
  var sab = (DB.ayarlar && DB.ayarlar.whatsappSablon && typeof DB.ayarlar.whatsappSablon === "object") ? DB.ayarlar.whatsappSablon : {};
...
function dersKartiUygun(d) {
  /* DONGU-15: iptal dersler ARTIK DISLANMAZ — gorsel kart butonu gorunur, PNG uretilebilir.
     WhatsApp METIN akisi degismedi: iptal filtresi ogrenciMesajMetni/waGonder’de aynen korunur. */
...
function dersKartiVeri(d) {
  ...
  /* DÖNGÜ-15 rozet sözleşmesi: 3 durum, sayı eki YASAK.
     planlandi/durumsuz → "Planlandı" · tamamlandi → "Yapıldı" · iptal → "İptal Edildi" */
...
function dersKartiHTML(d) {
  ...
  var durumStil = 'background:' + rozetRenk.bg + ';color:' + rozetRenk.fg + ';font-size:11px;font-weight:800;padding:4px 12px;border-radius:99px;white-space:nowrap';
  return '<div id="dersKartiGovde" style="width:640px;background:#f4f6fa;font-family:Inter,system-ui,sans-serif;padding:24px;border-radius:16px">' +
    '<div style="background:#ffffff;border-radius:14px;padding:24px 26px">' +
      /* Üst başlık: "Birebir Ders Kartı" + "Formül Kurs", sağ üstte durum rozeti */
      ...
      /* Öğrenci + sınıf bilgi alanı (slate-50 + indigo aksan) */
...
function dersKartiAc(dersId) {
  ...
  /* DÖNGÜ-15: bento ayarları — zemin rengi #f4f6fa; harici kaynak/canvas kirliliği kapalı; uydurma option YOK. */
  var bentoAyarlari = { scale: 2, backgroundColor: "#f4f6fa", useCORS: false, allowTaint: false, foreignObjectRendering: false, logging: false };
  var temizle = function () { var rapor = document.getElementById("dersKartiRapor"); if (rapor) { rapor.innerHTML = ""; } };
  html2canvas(el, bentoAyarlari)
    ...
    .catch(function () { temizle(); toast("Görsel oluşturulamadı.", "hata"); });
}
