...
for (const ad of Object.keys(manifest)) {
  ...
  let enst = fs.readFileSync(ad, "utf8");
  ...
  const a = !gecersizHit && hitSet.size >= 1 && kosumSatirlari >= 1;
...
console.log(hepsiOk ? "\nTAMLIK KANITI: 45/45 süitte her statik t( noktası koştu; vaka listesi tam." : "\nEKSİK VAR — yukarıda.");
