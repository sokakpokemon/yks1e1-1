...
function bransDersNorm(v) {
  ...
  return String(v || "").replace(/[ıİğĞüÜşŞöÖçÇ]/g, function (c) { return m[c]; }).replace(/[Iİ]/g, "i").toLocaleLowerCase("tr").replace(/\s+/g, "").trim();
}
...
function bransDersUygun(ogretmenId, dersId, konu) {
  ...
  var izinli = bransDersCoz(b);
  if (!izinli || !izinli.length) return false;
  return izinli.some(function (d) { return bransDersNorm(d) === bransDersNorm(dersId); });
}
/* BRANS-DERS-KURALI: branş-ad varyantları da aynı derse bağlanır */
var BRANS_AD_ES = { turkce: "tur", geometri: "geo", fizik: "fiz", kimya: "kim", biyoloji: "biy", edebiyat: "edb", tarih: "tar", ingilizce: "ing", cografya: "cgr", matematik: "mat" };
function bransDersCoz(b) {
  b = bransDersNorm(b);
  if (BRANS_AD_ES[b]) b = BRANS_AD_ES[b]; /* branş-adı varyantı → kanonik ders kodu */
  var haritaGirdi = Object.keys(BRANS_DERS_HARITA).filter(function (k) { return bransDersNorm(k) === b; })[0];
  if (haritaGirdi) return BRANS_DERS_HARITA[haritaGirdi];
  var eslesenDers = DERSLER.filter(function (d) { return bransDersNorm(d.id) === b; })[0];
  if (eslesenDers) return [eslesenDers.id];
  return null; /* FAIL-CLOSED: branş hiçbir derse bağlanamıyor */
}
...
/* DÖNGÜ-19: Birebir planlama + istek havuzu TEK ortak öğrenci etiketi formatter'ı.
   KURAL: grep ile bu tanım TAM 1 kez bulunmalı; hostlarda inline ad markup'ı yazılmaz.
   - sinif === null  → serbest metin istek (DB öğrencisi yok): sınıf spanı ÜRETİLMEZ
...
/* DÖNGÜ-19: TEK ortak custom suggestion satırı renderer'ı — TÜM autocomplete hostları bu renderer'ı kullanır.
   Kopya host markup'ı yasaktır. ogrenci=true ise ad + sınıf (birebirEtiketHTML); ogrenci=false ise yalnız ad. */
function ksSugSatirHTML(ad, sinif, ogrenci) {
  return ogrenci ? birebirEtiketHTML(ad, sinif) : '<span class="text-[12px] font-semibold normal-case tracking-normal text-slate-800 whitespace-normal break-words leading-snug">' + esc(gorselAd(ad)) + "</span>";
}
/* DÖNGÜ-19: Custom suggestion listesi — native datalist popup'ı CSS ile biçimlendirilemez;
   input'a list attribute'unu bırakmadan (native popup kapalı) aynı datalist değerlerinden
   filtreli, kompakt, sınıflı liste üretir. Input değeri HAM ad kalır; eşleşme mantığı değişmez. */
function ksSugListeHTML(items, tip) {
  return '<div class="ks-sug-liste hidden absolute z-30 left-0 right-0 top-full mt-1 max-h-60 overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-lg p-1">' +
    items.map(function (it) {
      return '<button type="button" class="ks-sug-satir w-full min-w-0 text-left px-3 py-2 text-[13px] font-medium rounded-lg hover:bg-teal-50/70 cursor-pointer flex whitespace-normal break-words" data-ks-sug="' + esc(it.deger) + '" data-ks-sug-hedef="' + esc(tip) + '">' +
        it.html + '</button>';
    }).join("") +
    (items.length ? "" : '<div class="px-3 py-2 text-[13px] font-medium text-slate-300">Eşleşme yok</div>') + "</div>";
}
/* DÖNGÜ-19: gorselAd — DB'deki ham adı render anında Türkçe güvenli normal harf düzenine çevirir.
   "SONER AÇIKGÖZ" → "Soner Açıkgöz"; İ/ı bozulmaz; DB ASLA değişmez. */
...
/* ---------- DONEM-ILK-YAMASI: 2026/2027 dönem modeli — yalnızca veri-uyumluluk dilimi ----------
...
function donemleriBaslat(db) {
  ...
  var say = { d: 0, ders: 0, ist: 0, ek: 0 }; /* EK-DERS-DONEM-YAMASI: ek ders backfill sayacı */
  ...
  /* DONEM-OLUSTURMA-YAMASI: çok dönemli DB'de işaretçi SIFIRLANMAZ — yalnız boşsa veya
  ...
  if (typeof sinifOgrtUyumOnar === "function") sinifOgrtUyumOnar(db); /* SINIF-OGRT-UYUM-YAMASI: loadDB/normalize yolu da kapatır */
  return say;
}
...
function aktifDonemId() {
  return (typeof DB !== "undefined" && DB && typeof DB.aktifDonemId === "string" && DB.aktifDonemId.trim() !== "") ? DB.aktifDonemId : "donem-2026-2027";
}
function bosDB() {
...
/* ---------- Kalıcı benzersiz ID altyapısı (KİMLİK-YAMASI) ----------
   Amaç: öğrenci/öğretmen/sınıf kayıtlarının kalıcı, çakışmasız kimlikleri.
   - ogrenciler[].id / ogretmenler[].id: zaten var olan UUID tabanlı id'ler — MEVCUT GEÇERLİ İD'LERE DOKUNULMAZ.
     Eksik/çakışan id'ler tek boot'ta üretilip KAYDEDİLİR; sonraki açılışlarda id değişmez (kalıcılık).
   - Sınıf kimliği: sinifProg anahtarları AD İLE aynı kalmaya devam eder (geriye dönük uyum);
     ek olarak DB.sinifIds[ad] = KIMLIK-ID haritası üretilir. sinifAdiDegistir/silme haritayı günceller.
   - Aynı isimli iki kayıt asla karışmaz: id üretimi kaydın KENDİSİNE (türü+sırası) bağlı, isme değil.
   - Referanslar (ders.ogrenciId/ogretmenId/ogrenciIds, istek.ogrenciId/ogrenciIds, ders.ogretmenId,
     ekDers.ogretmenId) normalize'da ASLA DEĞİŞTİRİLMEZ/TAŞINMAZ — kimlikler yalnızca eksik ID'leri tamamlar. */
function kimlikUret(tur, i) {
  return "ks-" + String(tur).slice(0, 3) + "-" + String(i + 1).padStart(4, "0") + "-" + Date.now().toString(36) + "-" + Math.random().toString(36).slice(2, 8);
}
/* DB'deki eksik/çakışan ID'leri tek geçişte tamamlar; IDEMPOTENT: geçerli id'lere ASLA dokunmaz.
   Dönüş: { o: eklenen öğrenci, t: eklenen öğretmen, s: eklenen sınıf } sayıları (değişiklik yoksa 0). */
function kimlikleriTamamla(db) {
  db = db || DB;
  if (!db || typeof db !== "object") return { o: 0, t: 0, s: 0 };
  var say = { o: 0, t: 0, s: 0 };
  function benzersiz(aday) {
    var k = String(aday), n = 2;
    function cakisiyor(h) { return db.ogrenciler.some(function (x) { return x.id === h; }) || db.ogretmenler.some(function (x) { return x.id === h; }) || (db.sinifIds && db.sinifIds[h] != null); }
    while (cakisiyor(k)) k = String(aday) + "-" + (n++);
    return k;
  }
  /* Öğrenci ve öğretmen id'leri — mevcut geçerli değerler KORUNUR (öncelik: sıradaki boot'ta aynen kalır) */
  (Array.isArray(db.ogrenciler) ? db.ogrenciler : []).forEach(function (o, i) {
    if (!o || typeof o !== "object") return;
    if (o.id == null || o.id === "") { o.id = benzersiz(kimlikUret("ogr", i)); say.o++; }
  });
  (Array.isArray(db.ogretmenler) ? db.ogretmenler : []).forEach(function (t, i) {
    if (!t || typeof t !== "object") return;
    if (t.id == null || t.id === "") { t.id = benzersiz(kimlikUret("ogr", i)); say.t++; }
  });
  /* Sınıf kimlikleri: sinifProg anahtarları + avail.sinif değerleri + ekDersler[].sinif
     İDEMPOTENT: sinifIds içinde zaten var olan ada DOKUNULMAZ (id tekrar çalıştırmada değişmez). */
  if (!db.sinifIds || typeof db.sinifIds !== "object" || Array.isArray(db.sinifIds)) db.sinifIds = {};
  var adlar = {};
  Object.keys(db.sinifProg || {}).forEach(function (s) { if (s) adlar[s] = 1; });
  (Array.isArray(db.ogrenciler) ? db.ogrenciler : []).forEach(function (o) { if (o && o.sinif) adlar[o.sinif] = 1; });
  if (Array.isArray(db.ekDersler)) db.ekDersler.forEach(function (l) { if (l && l.sinif) adlar[l.sinif] = 1; });
  if (Array.isArray(db.ogretmenler)) db.ogretmenler.forEach(function (t) {
    if (t && t.avail && t.avail.sinif && typeof t.avail.sinif === "object") Object.keys(t.avail.sinif).forEach(function (k) { var v = t.avail.sinif[k]; if (v && v !== "Sınıf Dersi") adlar[v] = 1; }); /* "Sınıf Dersi" placeholder'ı sınıf adı değildir */
  });
  Object.keys(adlar).forEach(function (ad) {
    if (db.sinifIds[ad] != null && db.sinifIds[ad] !== "") return; /* mevcut kimlik korunur */
    db.sinifIds[ad] = benzersiz(kimlikUret("snf", Object.keys(db.sinifIds).length));
    say.s++;
  }
  );
  return say;
...
function normalize(d) {
  ...
  if (Array.isArray(d.istekler)) d.istekler.forEach(function (r) {
    ...
    /* ISTEK-GRUP-ISTEGI-YAMASI-v2 (C): kayıpsızlık — temizlik sırayı/üyeyi değiştirmiyorsa DOKUNMA (yedek→yükleme deep-equal
  ...
  /* KİMLİK-YAMASI: eksik ID'leri burada tamamla — boot, yedek yükleme ve her saveDB öncesi aynı kapıdan geçer.
     kimlikleriTamamla idempotenttir; mevcut geçerli ID'lere (öğrenci/öğretmen id, sinifIds girdileri) dokunmaz. */
  kimlikleriTamamla(d);
...
/* ---------- Grup ders görünüm yardımcıları ----------
   Tek öğrencili derslerde eski görünüm birebir korunur (yardımcılar [] / boş döner).
   Grup derslerinde: ana öğrenci tam ad, diğer üyeler baş harf badge'i; 3+ üyede "+N" özeti.
   ui.grupAcikOgrId = ders.id → badge tıklanınca tüm adlar alt alta açılır. */
function grupOgrenciAdlari(ders) {
  return dersOgrenciIds(ders)
    .map(function (oid) { var o = DB.ogrenciler.find(function (x) { return x.id === oid; }); return o ? o.ad : ""; })
    .filter(function (ad) { return ad; });
}
function grupUyeEtiketleri(ders) {
  /* [ana ogrenciId, ...ek] sırasından: ana HARİÇ badge üyeleri (ek adları; ana formda tam ad) */
  if (!ders || !Array.isArray(ders.ogrenciIds) || !ders.ogrenciIds.length) return [];
  return ders.ogrenciIds
    .map(function (oid) { var o = DB.ogrenciler.find(function (x) { return x.id === oid; }); return o ? o.ad : ""; })
    .filter(function (ad) { return ad; });
}
function grupBadgeHTML(ders) {
  var uyeler = grupUyeEtiketleri(ders);
  ...
  if (uyeler.length > 2 && !acik) {
    html += '<button type="button" onclick="grupUyeToggle(\'' + esc(ders.id || "") + '\')" class="inline-flex items-center rounded-full bg-teal-50 border border-teal-200 px-1.5 py-0.5 text-[9.5px] font-bold text-teal-600 leading-none" title="' + esc(uyeler.map(function (a) { return a.split(" ")[0]; }).join(", ")) + '">+' + (uyeler.length - 2) + "</button>";
  } else if (uyeler.length > 2 && acik) {
    html += '<button type="button" onclick="grupUyeToggle(\'\')" class="inline-flex items-center rounded-full bg-slate-50 border border-slate-200 px-1.5 py-0.5 text-[9.5px] font-bold text-slate-400 leading-none" title="Daralt">−</button>';
  }
  return ' <span class="grup-badges inline-flex flex-wrap items-center gap-1 align-middle">' + html + "</span>";
}
function grupUyeToggle(dersId) {
  ui.grupAcikOgrId = dersId || null;
  renderDersler();
}
...
/* İdempotent okuma yardımcısı: istek kaydına ASLA yazmaz.
   ...
   - hatalı/boş kayıt → [] */
/* ISTEK-GRUP-ISTEGI-YAMASI-v2 (D): panel gövde markup'ı TEK kaynaktan — grupPanelCiz kapalıyken de aynı markup'ı üretir
...
function grupPanelGovdeHTML() {
  ...
  /* ISTEK-GRUP-ISTEGI-YAMASI-v3 (F): ASI düzeltmesi — return'den sonraki satır sonu ifadenin parçası
     olamaz; grupPanelGovdeHTML() undefined döndürüyordu, panel filtre markup'ı hiç çizilmiyordu */
  /* DÖNGÜ-21: arama kontrolü tam görünür — kapsayıcı kırpılma zinciri çözüldü: liste kapsayıcısındaki
     min-w-[280px] sabit genişlik dar host'ta yatay taşma üretip overflow-x-hidden ile arama satırını
     kırpıyordu; min-w-[280px] → w-full min-w-0 + input satırına min-w-0 eklendi (flex-1 + min-w-0 şartı).
     DÖNGÜ-18 kırpmasız liste düzeni korunur; gerçek taşma doğrulaması tarayıcıdadır (jsdom ölçmez). */
  return '<div class="flex flex-col sm:flex-row gap-2">' +
      '<div class="relative flex-1 min-w-0">' +
        '<input id="grup-panel-arama" type="text" autocomplete="off" placeholder="Öğrenci ara..." value="' + esc(ui.panelSecim.arama) + '" oninput="grupPanelAra(this.value)" class="w-full min-w-0 rounded-xl border border-slate-200 bg-white px-3 py-2 text-[13px] font-medium focus:outline-none focus:ring-2 focus:ring-teal-400/40" />' +
        '<i class="fa-solid fa-magnifying-glass absolute right-3 top-1/2 -translate-y-1/2 text-[11px] text-slate-300 pointer-events-none"></i>' +
      "</div>" +
      '<select id="grup-panel-sinif" onchange="grupPanelSinifSec(this.value)" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-[13px] font-medium text-slate-600 focus:outline-none focus:ring-2 focus:ring-teal-400/40">' + sinifOps + "</select>" +
    "</div>" +
    '<div id="grup-panel-liste" class="mt-2 w-full max-h-56 overflow-y-auto overflow-x-hidden rounded-xl border border-slate-200/70 bg-white divide-y divide-slate-100">' + grupPanelListeHTML() + '</div>';
}
...
function grupPanelListeHTML() {
  ...
  return liste.map(function (e) {
    ...
    /* DÖNGÜ-21: sınıf artık yalnız birebirEtiketHTML'den BİR kez gösterilir — eski ikinci sinifTag
       (text-[10px] font-bold text-slate-300) KALDIRILDI (çift gösterim); Ana rozeti ml-auto düzeni korunur. */
    return '<label class="flex items-start gap-3 px-3 py-2 hover:bg-teal-50/50 cursor-pointer' + (e.ana ? " bg-slate-50/80" : "") + '">' + kutu +
      birebirEtiketHTML(e.o.ad, e.o.sinif || "") + anaTag + "</label>";
  }).join("");
}
...
function istekGrupPanelAc() {
  ...
  ui.ekOgrenciIds = [];
  ui.havuzAnaId = null;
  ui.grupPanelSira = []; /* v3: havuz seçim sırası da sıfırlanır (eski üye yeni oturuma taşınmaz) */
  if (!ui.panelSecim) ui.panelSecim = { acik: false, arama: "", sinif: "" };
...
function istekGrupSifirla() {
  ...
  ui.ekOgrenciIds = [];
  ui.havuzAnaId = null;
  ui.grupPanelSira = []; /* v3: havuz seçim sırası da sıfırlanır */
...
function istekGrupOzetHTML() {
  ...
  var chipler = uyeler.map(function (oid, i) {
    ...
    return '<span class="inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold whitespace-normal break-words ' + (i === 0 ? "bg-slate-800 text-white" : "bg-teal-50 border border-teal-200 text-teal-700") + '">' + (o ? birebirEtiketHTML(o.ad, o.sinif || "") : "?") + (i === 0 ? ' <span class="opacity-70">(Ana)</span>' : "") + "</span>";
  }).join("");
...
/* KİMLİK-YAMASI: kimlik eksikleri localStorage'a YAZILMADAN önce kapanır → her kayıt kalıcı, benzersiz ID taşır.
   kimlikleriTamamla idempotenttir; zaten tam olan DB'de hiçbir alanı değiştirmez (JSON birebir aynı kalır). */
function kimlikKaydet() { kimlikleriTamamla(DB); return saveDB(); }
function loadDB() {
  try {
...
function sinifOgrtUyumOnar(db, tumDonemler) {
  /* SINIF-PROG-UYUM-GENISLETME: tumDonemler=true → onarım HER hedef dönemde çalışır (grid DB.sinifProg[ad] =
  ...
  if (!tumDonemler && hedef !== DONEM_ILK_ID) return { e: 0 }; /* normalize/loadDB yolu: dönemli programlara dokunmaz */
...
function seedDB() {
  ...
  db.ogretmenler = [
    ...
    { id: uid(), ad: "MERVE GEREK", brans: "mat", avail: { sinif: {"0-1":"11 SAY 3","1-1":"11 SAYCAL","2-1":"11 SAYISAL FEN","3-1":"11 EA 1"}, musait: ["4-1","5-1"] } },
    ...
    { id: uid(), ad: "SELİNA KUTLU", brans: "kim", avail: { sinif: {"0-1":"11 SAY 3","1-1":"11 SAYCAL","2-1":"11 SAYISAL FEN","3-1":"11 EA 1"}, musait: ["4-1","5-1"] } },
    ...
    { id: uid(), ad: "NİHAT KANARIĞ", brans: "tar", avail: { sinif: {"0-1":"12 DİL","1-1":"11 SAY 1","2-1":"11 SAY 2","3-1":"11 SAY 3"}, musait: ["4-1","5-1"] } },
    { id: uid(), ad: "MERT ASİL", brans: "ing", avail: { sinif: {"0-1":"11 SAY 3","1-1":"11 SAYCAL","2-1":"11 SAYISAL FEN","3-1":"11 EA 1"}, musait: ["4-1","5-1"] } }
  ];
  db.ogrenciler = [
  ...
  db.sinifProg = {
    "MEZUN SAY 1": ["0-1","2-1","3-1","5-1","0-2","4-1"],
...
/* DONEM-DOM-DÜZELTMESİ (idempotent ensureDonemUI): kök neden — ek-ders.js (defer) görevi
...
function sec(ad) { ui.sekme = ad; renderYonetim(); donemOnarimPlanla(); kartKolonOnar(); } /* DONEM-DOM-DÜZELTMESİ: alt sekme değişimlerinde de tek onarım planlanır (idempotent guard) · KART-KOLON-YAMASI: sekme değişiminde iki-kolon wrapper onarımı */
  ...
  if (secili) {
    ...
    grid = '<div class="rounded-2xl border border-slate-100 p-4 md:p-5">' +        durumSeciciHTML() +
        ...
        '<div class="flex items-center gap-3 text-[10.5px] font-semibold text-slate-500 flex-wrap">' +
          ...
          '<span class="flex items-center gap-1.5"><span class="w-3 h-3 rounded bg-slate-100 border border-slate-300 inline-block"></span> Kapalı</span> /* KAPALI-GORUNUM-YAMASI */' +
        "</div></div>" +
...
function gridTablo(onclickOnce, avail, tip, sinifAd) { /* DV-ETIKET-YAMASI */
  ...
  KISA_KOD.forEach(function (ks) {
    ...
    for (var g2 = 0; g2 < 7; g2++) {
      ...
      var durum = tip === "ogretmen"
        ? ((avail.sinif && key in avail.sinif) ? "sinif" : avail.musait.indexOf(key) >= 0 ? "kapali" : "") /* KAPALI-GORUNUM-YAMASI: musait → kapali dalına bağlandı (gri/soluk görünüm) */
      ...
      else if (durum === "kapali") { cls += "bg-slate-100 border-slate-200 hover:bg-slate-200 text-slate-400 opacity-70"; baslik += " · Kapalı"; } /* KAPALI-GORUNUM-YAMASI: kapalı hücre soluk/gri — tıklanabilir kalır */
      else if (durum === "var") { cls += "bg-blue-200 border-blue-300 hover:bg-blue-300 overflow-hidden"; var es5 = sinifAd ? dvSlotEtiketleri(sinifAd, key) : []; baslik += " · Toplu ders" + (es5.length ? " — " + es5.join(" · ") : ""); } /* DV-ETIKET-YAMASI */
      ...
      h += '<td class="p-0.5"><button class="' + cls + '" title="' + baslik + '" onclick="' + onclickOnce + "," + g2 + "," + ks.no + ')">' +
        (durum === "sinif" ? '<span class="text-[7.5px] font-extrabold text-amber-700/80 leading-tight whitespace-nowrap">' + esc((avail.sinif[key] || "SD").substring(0, 14)) + '</span>' :
         durum === "kapali" ? "" : /* KAPALI-GORUNUM-YAMASI: "K" kırmızı etiketi kaldırıldı — Kapalı bilgisi title'da */
         durum === "var" ? (function () { var es = dvSlotEtiketleri(sinifAd, key); return es.length ? '<span class="block min-w-0 max-w-[72px] mx-auto px-0.5 text-[7.5px] font-extrabold text-blue-700/80 leading-tight truncate text-left">' + esc(es.join(" · ")) + '</span>' : ""; })() : "") + /* DV-ETIKET-YAMASI: DV kaldırıldı; kaynaklı eşleşme yoksa uydurma YOK */
        "</button></td>";
    }
    h += "</tr>";
  });
...
function ogrenciTab() {
  ...
  if (ui.sinifAd) {
    ...
    prog = '<div class="rounded-2xl border border-slate-100 p-4 md:p-5">' +
      ...
      gridTablo("togSinif('" + esc(ui.sinifAd).replace(/'/g, "\'") + "'", program, "sinif", ui.sinifAd) + "</div>"; /* DV-ETIKET-YAMASI */
  } else {
...
function sinifEkle() {
  ...
  DB.sinifProg[ad] = [];
  /* KİMLİK-YAMASI-BÖLGE2: yeni sınıfa kalıcı kimlik (sinifIds) */
...
function sinifAdiDegistir() {
  ...
  /* KİMLİK-YAMASI-BÖLGE2: sınıf kimliği ada değil kayda bağlı — yeniden adlandırmada KORUNUR */
...
function sinifSil() {
  ...
  onayAc({
    ...
    /* KİMLİK-YAMASI-BÖLGE2: silinen sınıfın kimliği de arşivden kalkar */
...
function csvImportUygula(dosyalar) {
  ...
  dosyalar.forEach(function (d) {
    ...
    if (c.dataset === "sinifProg") { hatalar.push(csvHata(d.ad, 1, "dataset", "sinifProg dataset'i genel içe aktarmada desteklenmez — Sınıf Programı CSV İçe Aktar butonunu kullanın")); return; }
  ...
  cozulen.filter(function (c) { return c.dataset === "kadro"; }).forEach(function (c) {
    ...
    c.veriler.forEach(function (v, idx) {
      ...
      if (!id) {
        /* Eksik ID üretimi (UPSERT-1.5): öğrenci/öğretmen → kimlikUret + benzersiz savunması;
           sınıf → ada göre mevcut sınıfa bağlan, yoksa kadroSnfId ile üret (deterministik) */
        if (tip === "sinif") {
          var _snfAd = (v.ad || v.sinifAd || "").trim();
          var _snfMevcut = kopya.sinifIds[_snfAd];
          if (_snfMevcut != null && _snfMevcut !== "") id = _snfMevcut;
          else { id = (typeof kadroSnfId === "function" ? kadroSnfId(_snfAd) : ("ks-snf-" + _snfAd)); uretilen++; }
        } else if (tip === "ogrenci" || tip === "ogretmen") {
          var _aday = kimlikUret("ogr", tip === "ogrenci" ? kopya.ogrenciler.length : kopya.ogretmenler.length);
          var _n = 2;
          var _cak = function (h) { return kopya.ogrenciler.some(function (x) { return x.id === h; }) || kopya.ogretmenler.some(function (x) { return x.id === h; }); };
          while (_cak(_aday)) _aday = kimlikUret("ogr", 9999) + "-" + (_n++);
          id = _aday;
          uretilen++;
        } else { hatalar.push(csvHata("kadro", sn, "id", "ID boş")); return; }
      }
  ...
  cozulen.filter(function (c) { return c.dataset === "dersler" || c.dataset === "istekler"; }).forEach(function (c) {
    ...
    c.veriler.forEach(function (v, idx) {
      ...
      /* istek kayıtlarında ogretmenId alanı yok — yalnız derslerde zorunlu; doluysa yine doğrulanır */
      if (isDers && (!ortId || !ortSet[ortId])) { hatalar.push(csvHata(c.dataset, sn, "ogretmenId", "Bilinmeyen öğretmen ID: " + (ortId || "(boş)"))); return; }
      if (!isDers && ortId && !ortSet[ortId]) { hatalar.push(csvHata(c.dataset, sn, "ogretmenId", "Bilinmeyen öğretmen ID: " + ortId)); return; }
      ...
      if (mevcutKayit) {
        ...
        if (isDers && !ortId) { /* ders için ogretmenId zaten zorunlu — buraya gelmez */ }
...
/* ================= DONEM-OLUSTURMA-YAMASI: dönemli sınıf programı + yeni dönem =================
...
function renderHavuz() {
  ...
  /* DÖNGÜ-28-YAMASI: kart listesi geniş ekranda İKİ SÜTUN grid (zigzag: 1 sol, 2 sağ, 3 sol…);
  ...
  if (!sirali.length) {
    liste += '<div class="md:col-span-2 border border-dashed border-slate-200 rounded-2xl py-10 text-center"><p class="text-[12px] text-slate-400">' + (aktif ? "Bu derse ait istek yok." : "Havuz boş — rehberlik servisinin öğrenci talepleri burada toplanır.") + "</p></div>";
  }
  sirali.forEach(function (r, i) {
    ...
    liste += '<div draggable="' + bekliyor + '" data-istek="' + r.id + '" class="istek-kart flex items-center gap-3 rounded-xl border px-3.5 py-2.5 ' + (bekliyor ? "border-slate-100 hover:border-teal-300 transition-colors cursor-grab active:cursor-grabbing" : "border-green-100 bg-green-50/40") + '"' +
      ...
      (D2 ? '<span class="rounded-full px-2 py-0.5 text-[11px] font-semibold ' + D2.bg + " " + D2.tx + '">' + D2.ad + "</span>" : "") +
      '<span class="text-[10px] font-medium rounded-full px-2 py-0.5 ' + (bekliyor ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700") + '">' + (bekliyor ? "Bekliyor" : "Planlandı") + "</span></div>" +
      '<div class="text-[10px] font-medium text-slate-500 whitespace-normal break-words mt-0.5">' + (r.konu ? "<i>Eksik konu:</i> " + esc(r.konu) : '<span class="italic text-slate-300">Konu belirtilmedi</span>') + "</div>" +
      '<div class="text-[10px] font-medium text-slate-400 mt-0.5"><i class="fa-regular fa-calendar mr-1"></i>' + fmtTR(r.olusturma) + (r.saat ? " · <i class=\'fa-regular fa-clock ml-1 mr-1\'></i>" + saatEtiket(r.saat) : "") + "</div></div>" +
      (bekliyor
  ...
  $("havuzBolum").innerHTML =
    '<div class="kart p-5">' +
      ...
      '<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 mb-4">' +
        /* DÖNGÜ-22: h-ogrenci KENDİ relative sarmalayıcısında — öneri popup'ı grid'e değil input'a ankrajlı (sol-üst yüzme düzeltmesi) */
        '<div class="relative min-w-0"><input id="h-ogrenci" list="dl-ogrenci" placeholder="Öğrenci adı" autocomplete="off" class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-[13px] font-medium focus:outline-none focus:ring-2 focus:ring-teal-400/40" /></div>' +
        '<select id="h-ders" class="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-[13px] font-medium focus:outline-none focus:ring-2 focus:ring-teal-400/40">' + dersOps + "</select>" +
        '<input id="h-konu" placeholder="Eksik konu (örn. Paragraf)" autocomplete="off" class="lg:col-span-2 rounded-xl border border-slate-200 px-3 py-2.5 text-[13px] font-medium focus:outline-none focus:ring-2 focus:ring-teal-400/40" />' +
...
function istekEkle() {
  ...
  DB.istekler.push({ id: uid(), ogrenciId: o.id, ogrenciAd: o.ad, dersId: dersId, konu: konu, durum: "bekliyor", olusturma: todayKey(), donemId: aktifDonemId() /* DONEM-DAMGA-YAMASI: yeni istek aktif döneme damgalanır */ });
...
/* DÖNGÜ-19: Custom suggestion — native datalist popup'ı biçimlendirilemez; TÜM hostlar
   (birebir planlama öğrenci/öğretmen + havuz öğrenci) TEK renderer (ksSugSatirHTML) ile listelenir.
   Input değeri HAM ad kalır; list="..." kaldırılmadığı için native davranış da bozulmaz (listeler gizli). */
function ksSugOgrenci(q) {
  var s = kucuk(q || "");
  return DB.ogrenciler.filter(function (o) { return !s || kucuk(o.ad).indexOf(s) !== -1; })
    .map(function (o) { return { deger: o.ad, html: birebirEtiketHTML(o.ad, o.sinif || "") }; });
}
function ksSugOgretmen(q) {
  var s = kucuk(q || "");
  return DB.ogretmenler.filter(function (t) { return !s || kucuk(t.ad).indexOf(s) !== -1; })
    .map(function (t) { return { deger: t.ad, html: ksSugSatirHTML(t.ad, null, false) }; });
}
/* DÖNGÜ-21: özel öneri listesi scroll/resize ANKRAJI — liste input'a sabitlenir; scroll/resize'da
   yeniden konumlanır; yatay kayma yok (genişlik host rect'ten hesaplanır). jsdom gerçek layout
   ölçmediğinden geometri doğrulaması tarayıcıdadır; assertion'lar listener/kod yapısı üzerinden. */
function ksSugAnkraj(inpId, tip) {
  /* DÖNGÜ-22: popup in-flow absolute (top-full, left-0, right-0) — relative host ile OTOMATİK ankrajlı;
     scroll/resize JS konumlandırması gereksiz (kayma kanıtı gerektirmez). Fonksiyon uyumluluk için korunur. */
  void inpId; void tip;
}
function ksSugCiz(inpId, tip) {
  var inp = $(inpId); if (!inp) return;
  var liste = tip === "ogretmen" ? ksSugOgretmen(inp.value) : ksSugOgrenci(inp.value);
  var host = inp.parentNode; if (!host) return;
  var el = document.getElementById("ks-sug-" + tip);
  if (!el) {
    if (!host.insertAdjacentHTML) return;
    host.insertAdjacentHTML("beforeend", ksSugListeHTML(liste, tip));
    el = document.getElementById("ks-sug-" + tip);
    if (el && !inp.dataset.ksSug) {
      inp.dataset.ksSug = "1";
      inp.addEventListener("focus", function () { ksSugCiz(inpId, tip); if (el) el.classList.remove("hidden"); ksSugAnkraj(inpId, tip); });
      inp.addEventListener("input", function () { ksSugCiz(inpId, tip); if (el) el.classList.remove("hidden"); ksSugAnkraj(inpId, tip); });
      inp.addEventListener("blur", function () { setTimeout(function () { if (el) el.classList.add("hidden"); }, 160); });
    }
    if (el) el.addEventListener("mousedown", function (e) {
      var b = e.target && e.target.closest ? e.target.closest("[data-ks-sug]") : null;
      if (b && inp) { inp.value = b.getAttribute("data-ks-sug"); if (tip === "ogrenci") duzenleBannerGuncelle(); if (el) el.classList.add("hidden"); e.preventDefault(); }
    });
  } else {
    el.innerHTML = ksSugListeHTML(liste, tip).replace(/^<div[^>]*>/, "").replace(/<\/div>$/, "");
  }
}
function renderFormDestek() {
  ...
  /* DÖNGÜ-19: custom suggestion hostları — birebir planlama (f-ogrenci/f-ogretmen) + havuz (h-ogrenci) */
  ["f-ogrenci:ogrenci", "h-ogrenci:ogrenci", "f-ogretmen:ogretmen"].forEach(function (p) {
    var par = p.split(":"); var inp = $(par[0]);
    if (!inp) return;
    var host = inp.parentNode; if (!host) return;
    var st = host.style || {};
    if (!host.className || host.className.indexOf("relative") === -1) {
      if (host.classList && host.classList.add) { host.classList.add("relative"); }
    }
    ksSugCiz(par[0], par[1]);
  });
  ...
  var ekPanel = '<div id="ek-ogrenciler" class="no-print rounded-2xl border border-slate-100 bg-slate-50/60 p-3 mt-2">' +
    '<div class="flex items-center justify-between gap-2">' +
      '<span class="text-[9px] font-bold text-slate-400 normal-case tracking-normal"><i class="fa-solid fa-user-group mr-1"></i>Grup Öğrencileri</span>' +
...
function grupPanelOzetCiz() {
  ...
  if (!ozetEl) return;
  if (!ui.panelSecim) ui.panelSecim = { acik: false, arama: "", sinif: "" };
  /* ISTEK-GRUP-ISTEGI-YAMASI-v2 (E): havuz bağlamında plan tarafı yan etkisi YOK (ek listeye kopya yazılmaz) */
  ...
  var chipler = secili.filter(function (oid) { return oid !== anaId; }).map(function (oid) {
    var o = DB.ogrenciler.find(function (x) { return x.id === oid; });
    return '<span class="inline-flex items-center gap-1.5 rounded-full bg-teal-50 border border-teal-200 pl-2.5 pr-1.5 py-1 text-[11px] font-semibold text-teal-700 whitespace-normal break-words">' + (o ? birebirEtiketHTML(o.ad, o.sinif || "") : "?") +
...
function grupPanelCiz() {
  ...
  /* ISTEK-GRUP-ISTEGI-YAMASI-v2 (B): kapalıyken markup KORUNUR (yalnızca gizlenir) — arama kutusu id'si DOM'da kalır;
...
function duzenleBannerGuncelle() {
  ...
  if (ui.editId) {
    ...
    if (l) {
      ...
      banner.innerHTML = '<i class="fa-solid fa-pen"></i>Ders düzenleniyor: ' + (function(){ var _o=DB.ogrenciler.find(function(x){return x.ad===l.ogrenciAd;}); return birebirEtiketHTML(l.ogrenciAd, _o ? (_o.sinif||"") : null); })() + " · " + fmtTR(l.tarih) + " " + saatEtiket(l.saat);
...
function duzeltmeBul(adet, yokSay, grupOgrenciIds) {
  ...
  if (ogr) {
    ...
    if (cakisan) uyari.push("Aynı kısa kod saatinde " + ogr.ad + " öğretmeninin <b>" + esc(gorselAd(cakisan.ogrenciAd)) + "</b> ile başka bir dersi var (" + fmtTR(cakisan.tarih) + " " + saatEtiket(cakisan.saat) + ").");
  ...
  if (o && o.sinif && (DB.sinifProg[o.sinif] || []).indexOf(key) >= 0) {
    uyari.push(gorselAd(o.ad) + " öğrencisinin sınıfı (<b>" + o.sinif + "</b>) o kısa kod saatinde toplu derste.");
  }
    ...
    for (var hi = 0; hi < HAFTA_SLOTLARI.length; hi++) {
      ...
      if (musaitDegil) { /* PAZAR-BIREBIR-GORUNUM-YAMASI: Pazar artık normal gün satırı — yalnız Kapalı (musaitDegil) gri kalır */
        ...
        satirlar += '<td class="dnd-kilit px-1.5 py-1.5 text-center border-l border-slate-100"' +
          /* DÖNGÜ-29-YAMASI: grup birebir dersler de draggable — yalnız havuz hedefi kabul eder (dersBurak reddeder); iptal/tamamlanmış draggable değil */
...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
/* DÖNGÜ-29-YAMASI — ÇİZELGEDEN HAVUZA GERİ BIRAKMA (onaylı)
...
function gunlukTablo() {
  ...
  ogrtSirasi.forEach(function (ogrtAd, oi) {
    ...
    // Öğretmen adı
    /* OGRT-TAMGUN-KART: buton yalnız günlük tam program satırı VARSA (sınıf dersi VEYA birebir) öğretmen adı hücresinde çizilir */
    ...
    html += '<td class="px-3 py-3 border-r border-slate-200 text-left">' +
      '<div class="text-[12px] font-black text-slate-800 uppercase leading-tight">' + esc(ogrtAd) + '</div>' +
  ...
  liste.forEach(function (l) {
    ...
    satirlar += '<tr class="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">' +
      ...
      '<td class="px-4 py-3"><div class="flex items-center gap-1">' +
        ...
        (dersKartiUygun(l) ? '<button title="Ders Kartı görseli (PNG) üret" draggable="false" onmousedown="event.stopPropagation()" onclick="event.preventDefault();dersKartiAc(\'' + l.id + '\')" class="w-8 h-8 rounded-full text-violet-400 hover:text-violet-600 hover:bg-violet-50 transition-colors"><i class="fa-solid fa-image text-[12px]"></i></button>' : '') + /* DERS-KARTI-TASIMA-YAMASI: kart butonu ISLEM alanına taşındı (waSatir yanına) */
...
function ogrenciMesajMetni(ogrenciId) {
  ...
  var satirlar = liste.map(function (l, i) {
    ...
    /* D25-SABLON-YAMASI: birebir öğrenci mesajı onaylı yeni şablon —
       1. {DERS} ({OGRETMEN})
       {TARİH} {GÜN}
       {SAAT-BAS} - {SAAT-BİT}
       Konu satırı, grup üye satırı ve ders-başı durum cümlesi bilinçli olarak KALDIRILDI;
       kapanış tek paragraf bloğu + imza. Emoji YOK; slot numarası YOK — saat aralığı
       KISA_KOD'dan gerçek başlangıç-bitiş olarak üretilir. saatEtiket() DEĞİŞMEDİ. */
    var ogrAd = l.ogretmenAd;
    if (!ogrAd && l.ogretmenId && DB.ogretmenler) {
      var og = DB.ogretmenler.find(function (x) { return x.id === l.ogretmenId; });
      if (og) ogrAd = og.ad;
    }
    var kk = KISA_KOD.filter(function (x) { return x.no === ksKodOf(l.saat); })[0];
    var saatAralik = kk ? kk.b + " - " + kk.e : String(l.saat || "");
    return (i + 1) + ". " + D.ad + (ogrAd ? " (" + ogrAd + ")" : "") +
      "\n" + fmtTR(l.tarih) + " " + GUNLER[dowIdx(l.tarih)] +
      "\n" + saatAralik;
  });
  /* D25: onaylı tek kapanış bloğu + imza; emoji yok. */
  var kapanis =
    "Bu dersler, eksiklerini tamamlaman ve hedeflerine biraz daha yaklaşman için planlandı.\n" +
    "Ders saatinden birkaç dakika önce hazır olman yeterli. Anlamadığın veya zorlandığın konuları öğretmeninle paylaşmayı unutma.\n" +
    "Sen çalışmaya devam et, biz de bu süreçte yanında olalım.\n" +
    "Güzel çalışmalar, başarılar dileriz.\n" +
    "— FORMÜL KURS REHBERLİK SERVİSİ";
  return "Değerli öğrencimiz " + o.ad + ",\nBirebir ders programın aşağıdaki gibidir:\n\n" + satirlar.join("\n") + "\n\n" + kapanis;
}
/* WA-ALICI-YAMASI: modal-genel alici secici (bellek içi state — localStorage'a YAZILMAZ).
...
function waAc() {
  ...
  waAliciPanelGuncelle();
  /* WA-ONIZLEME-YAMASI: modal her açılışta onizleme panelini placeholder'a sıfırla (duplicate yok, tek panel) */
  waOnizlemeSifirla();
  $("waModal").classList.remove("hidden");
}
function waKapat() {
  $("waModal").classList.add("hidden");
  /* WA-ALICI-YAMASI: state temizle — alici seçimi yalnız bellek içi, kalıcı kayıt YOK */
  waAktifOgrenciId = null;
  waAliciTipi = "ogrenci";
}

/* WA-ONIZLEME-YAMASI: Ortak tekil onizleme paneli (id sabit; mesaj metni ASLA burada yeniden üretilmez,
...
function waOnizle(ogrenciId) {
  ...
  var o = DB.ogrenciler.find(function (x) { return x.id === ogrenciId; });
  waAktifOgrenciId = ogrenciId; /* WA-ALICI-YAMASI: aktif öğrenci takibi (alıcı seçimi korunur) */
  waAliciPanelGuncelle();
...
function waGonder(ogrenciId) {
  ...
  var a = waAliciBilgisi(ogrenciId, waAliciTipi); /* WA-ALICI-YAMASI: seçili alıcı — fallback YOK */
...
/* DERS-KARTI-YAMASI-V2: buton td drag'ini TETİKLEMEZ —
...
function dersKartiVeri(d) {
  ...
  /* DÖNGÜ-16: kart-kapsamlı saat biçimleyici — slot numarası YOK ("15:30-16:10").
     Ortak saatEtiket'e dokunulmaz (WA/öğretmen "8 · " gösterimini korur); TEK çağrı noktası burası.
     KISA_KOD eşleşmezse fallback String(saat) — saatEtiket fallback'iyle aynı sözleşme. */
  var kartSaati = (function (saat) {
    var m = KISA_KOD.filter(function (x) { return x.no === ksKodOf(saat); })[0];
...
function dersKartiHTML(d) {
  ...
  return '<div id="dersKartiGovde" style="width:640px;background:#f4f6fa;font-family:Inter,system-ui,sans-serif;padding:24px;border-radius:16px">' +
    '<div style="background:#ffffff;border-radius:14px;padding:24px 26px">' +
      ...
      "</div>" +
      /* DÖNGÜ-16: öğrenci kartı footer KALDIRILDI (öğretmen kartlarındaki footer korunur) */
    "</div>" +
  "</div>";
}
...
function dersKartiAc(dersId) {
  ...
  function dosyaAdi() {
    ...
    var v = dersKartiVeri(d);
    /* DERS-KARTI-YAMASI-V2: Türkçe transliterasyon + "/" ":" gibi yol/istek zararlılarını kaldır */
  ...
  html2canvas(el, { scale: 2, backgroundColor: "#f4f6fa", useCORS: false, allowTaint: false, foreignObjectRendering: false, logging: false })
    .then(function (canvas) {
      canvas.toBlob(function (blob) {
        ...
        /* DERS-KARTI-YAMASI-V2: file:// → pano denenmez; izin reddi → catch indir yoluyla biter */
...
/* OGRT-KART-YAMASI: öğretmen modu GÜNLÜK birebir ders görseli (öğrenci hattının aynısı, öğretmen akışı).
   - Öğrenci modu (dersKartiVeri/HTML/Ac/BtnHTML) BİREBİR korunur; çağrıları/metinleri/dosya adı değişmez.
   - Sınıf kaynağı yalnız DB.ogrenciler[].sinif (boş → "Sınıf belirtilmemiş"); sinifProg/avail/ekDers KULLANILMAZ.
   - HARIC (buton çizilmez, akış girmez): grup (>1 öğrenci), Sınıf Dersi (öğrenci boş), Ek Ders, iptal — dersKartiUygun dışlar.
   - Görselde TELEFON YOK (ne öğrenci ne öğretmen); dosya adında telefon yok.
   - WA hedefi "ogretmen" YALNIZ bu akışta okunur; öğretmen tel yoksa PNG yine iner.
   - Bu özellik VERİ DEĞİŞTİRMEZ: saveDB/localStorage yazımı YOK. */
function dersKartiOgrtVeri(d) {
  var v = dersKartiVeri(d);
  return { ogrAd: v.ogr, ad: v.ad, sinif: v.sinif || "Sınıf belirtilmemiş", ders: v.ders, konu: v.konu || "Genel tekrar", saat: v.saat, tarih: v.tarih, durum: d.durum === "tamamlandi" ? "Yapıldı" : "Planlandı" };
}
function dersKartiOgrtHTML(d) {
  var v = dersKartiOgrtVeri(d);
  function satir(b, x) { return '<div style="display:flex;gap:10px;font-size:14px;color:#334155;padding:5px 0"><span style="width:110px;font-size:10px;font-weight:800;letter-spacing:.06em;color:#94a3b8;padding-top:3px">' + b + "</span><span style=\"font-weight:600\">" + esc(x) + "</span></div>"; }
  return '<div id="dersKartiGovde" style="width:640px;background:#fff;font-family:Inter,system-ui,sans-serif;padding:34px 40px;border-radius:0">' +
    ...
    '<div style="font-size:22px;font-weight:800;color:#0f172a">' + esc(v.ogrAd) + "</div>" +
    satir("SAAT", v.saat) + satir("ÖĞRENCİ", v.ad) + satir("SINIF", v.sinif) +
    satir("DERS", v.ders) + satir("KONU", v.konu) + satir("TARİH", v.tarih) +
...
function dersKartiOgrtBtnHTML(d) {
  return '<button draggable="false" title="Günlük ders görseli (PNG) üret" ' +
    'onmousedown="event.stopPropagation()" onclick="event.stopPropagation();event.preventDefault();dersKartiOgrtAc(\'' + esc(d.id) + '\')" ' +
...
function dersKartiOgrtAc(dersId) {
  var d = DB.dersler.find(function (x) { return x.id === dersId; });
  if (!d) { toast("Ders bulunamadı.", "hata"); return; }
  if (!dersKartiUygun(d)) { toast("Görsel yalnız aktif tek öğrencili birebir ders için üretilir.", "uyari"); return; }
  if (!d.tarih) { toast("Ders tarihi bilinmiyor; görsel üretilmedi.", "uyari"); return; }
  if (!window.html2canvas) { toast("Görsel motoru (html2canvas) yüklenemedi. İnternet bağlantısını kontrol edip sayfayı yenileyin.", "hata"); return; }
  /* OGRT-KART: WA hedefi (ogretmen) yalnız bu akışta okunur — ogrenci/anne/baba yolları birebir korunur.
     Öğretmen tel yoksa: otomatik alıcı/WA hedefi EKLENMEZ, öğrenci telefonuna fallback YASAK; PNG yine iner. */
  var a = waAliciBilgisi(d.ogretmenId || "", "ogretmen");
  var guvendemi = !(typeof window.isSecureContext === "boolean" && window.isSecureContext === false);
  var el = document.getElementById("dersKartiRapor");
  if (!el) {
    el = document.createElement("div");
  ...
  el.innerHTML = dersKartiOgrtHTML(d);
  function dosyaAdi() {
    /* GIZLILIK: dosya adında TELEFON YOK — öğretmen adı + tarih */
    var v = dersKartiOgrtVeri(d);
    return "ders-karti-ogretmen-" + String(v.ogrAd)
      .replace(/ğ/g, "g").replace(/Ğ/g, "g").replace(/ü/g, "u").replace(/Ü/g, "u")
      .replace(/ş/g, "s").replace(/Ş/g, "s").replace(/ı/g, "i").replace(/İ/g, "i")
      .replace(/ö/g, "o").replace(/Ö/g, "o").replace(/ç/g, "c").replace(/Ç/g, "c")
      .replace(/[^a-zA-Z0-9]+/g, "-").replace(/^-+|-+$/g, "")
...
function ogrtGunlukSatirlar(ogrtId, gunKey) {
  ...
  /* Çakışma: aynı slotta sınıf dersi + birebir → her ikisine işaret */
  var slotSay = {};
  satirlar.forEach(function (r) { slotSay[r.slot] = (slotSay[r.slot] || 0) + 1; });
...
function dersKartiOgrtGunlukAc(ogrtId, gunKey) {
  ...
  var satirlar = ogrtGunlukSatirlar(ogrtId, gunKey);
  if (!satirlar.length) { toast("Bu gun icin paylasilabilir ders yok.", "uyari"); return; }
  if (!window.html2canvas) { toast("Görsel motoru (html2canvas) yüklenemedi. İnternet bağlantısını kontrol edip sayfayı yenileyin.", "hata"); return; }
...
setTimeout(donemOzDenetim, 0);
document.addEventListener("keydown", function (e) {
  if (e.key === "Escape") { onayKapat(); waKapat(); pngKapat(); }
});

/* EXCEL-UI-YAMASI: csvYonetimKartHTML içindeki çift artı (unary + → NaN) giderildi ve EK-DERS satırına eksik artı eklendi. Etiket/buton/CSV davranışı değişmedi. */