...
process.on("exit", (c) => { if (c !== 0) return; if (__kosan !== 51) { console.error("SUITE_DONE UYUŞMAZLIĞI: ks-ekders-gorunum.mjs kosan=" + __kosan + " beklenen=51"); process.exitCode = 1; } else { console.log("SUITE_DONE:ks-ekders-gorunum.mjs:" + __kosan + ":51"); } });
...
if (seedDers) {
  globalThis.__ekdersSeedDali = true; /* GERÇEK-DAL: if dalı fiilen koştu */
...
t("ekders fixture: seed-ders if dalı GERÇEKTEN koştu (seedDers dolu → if t'leri üretildi)", globalThis.__ekdersSeedDali === true && !!seedDers);
